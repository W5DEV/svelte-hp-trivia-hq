<script lang="ts">
	...
	$: displayedAnswers = question.answers
		.map((answer) => ({ answer, selected: false, sort: Math.random() }))
...
<div class="flex items-center justify-center w-screen font-caveat">
	<div
		class="flex flex-col items-center justify-between w-screen max-w-4xl gap-6 px-10 md:px-28 py-16 min-h-[600px] h-content"
		style="background-image: url({parchment}); background-size: cover; background-position: center; background-repeat: no-repeat;"
	>
		...
		<div class="flex flex-col items-center justify-center w-full gap-4 text-left">
			<h2 class="text-3xl font-semibold text-gryffindor-red">
				Question {questionNumber} of {totalQuestions}:
			</h2>
			<h2 class="w-full text-4xl font-light text-left text-gryffindor-red">
				<strong class="font-extrabold underline"
		...
		{#if question.type === 'multi-select'}
			<div class="flex flex-col items-start justify-center w-full gap-4 font-light text-left">
				{#each displayedAnswers as option, i}
					<button
						on:click={() => handleMultiSelection(option, i)}
						class={displayedAnswers[i].selected
							? `text-left text-slytherin-green font-light text-3xl hover:italic px-4`
							: `text-left text-gryffindor-red font-light text-3xl hover:bg-transparent hover:italic px-4`}
					>
			...
			<div class="flex flex-row items-center justify-center w-full">
				<button
					...
					class="text-4xl btn btn-ghost btn-wide text-gryffindor-red hover:text-slytherin-green hover:bg-transparent"
					>Next</button
				>
			</div>
		{:else}
			<div class="flex flex-col items-start justify-center w-full gap-4 font-light text-left">
				{#each displayedAnswers as option, i}
					<button
						on:click={() => handleSelection(option, i)}
						class={displayedAnswers[i].selected
							? `text-left text-slytherin-green font-light text-3xl hover:italic px-4`
							: `text-left text-gryffindor-red font-light text-3xl hover:bg-transparent hover:italic px-4`}
					>
		...
		<div class="flex flex-row items-end justify-end w-full top-10 right-10">
			<a
				href={getOriginLink()}
				target="_blank"
				class="text-sm font-medium tooltip tooltip-left text-gryffindor-red"
				data-tip={question.question_origin}
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					target="_blank"
