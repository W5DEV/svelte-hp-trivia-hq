<script lang="ts">
    ...
    const difficulty = question.difficulty ? Math.floor(question.difficulty/10) : 5;
</script>
...
<div class="flex items-center justify-center w-screen font-caveat">
    <div class="flex flex-col items-center justify-between w-screen max-w-4xl gap-6 px-10 md:px-28 py-24 min-h-[600px] h-content" style="background-image: url({parchment}); background-size: cover; background-position: center; background-repeat: no-repeat;">
        <div class="flex flex-row items-center justify-center w-full gap-2 mb-10 -mt-10">
            <p class="mr-1 text-2xl font-medium text-neutral">Difficulty:</p>
            <span class={'text-2xl font-semi-bold text-neutral ' + (difficulty < 3 ? 'text-green-600' : difficulty > 7 ? 'text-red-600' : 'text-yellow-400')}>
                {difficulty}/10
            </span>
            <span class={(difficulty < 3 ? 'text-green-600' : difficulty > 7 ? 'text-red-600' : 'text-yellow-400')}>
        ...
        <div class="flex flex-row items-end justify-end w-full top-10 right-10">
            <a href={getOriginLink()} class="text-sm font-medium tooltip tooltip-left text-info" data-tip={question.question_origin}>
