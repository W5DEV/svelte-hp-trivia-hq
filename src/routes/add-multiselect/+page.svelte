<script lang="ts">
    ...
    $: questionOriginsActive = questionOrigins.filter((origin) => origin.status === "in-progress");
...
<section class="flex flex-col items-center justify-center flex-1 w-full gap-12 py-16">
    <h1 class="text-2xl font-medium text-primary">Submit a new Multi Select Question</h1>
	{#if !showSubmit}
		<form
			use:form
			...
			<div class="flex flex-col items-center justify-center w-full max-w-3xl gap-4 px-8 md:px-12">
                ...
                >
                    ...
                    {#each questionOriginsActive as origin}
                        <option value={origin.source}>{origin.source}</option>
                    {/each}
                </select>
